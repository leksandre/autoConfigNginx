...
def processFile(path, fileName):
    ...
    if find_phrases(path+fileName, 'ssl_certificate /etc/letsencrypt/live/domen3.domen2.domen1/fullchain.pem;'):
        if not(os.path.isfile('/home/mobsted3/docker_mobsted/conf/letsencrypt/live/'+('.'.join(fparts))+'/fullchain.pem')):
            ...
            reloadNginx()  # рано
            os.chdir('/home/mobsted3/docker_mobsted/')
            res = generateConfignginx('.'.join(fparts))
            print(res)
            if res.find('Congratulations! Your certificate and chain have been saved at') > -1 or (os.path.isfile('/home/mobsted3/docker_mobsted/conf/letsencrypt/live/'+('.'.join(fparts))+'/fullchain.pem')):
...
directory = '/home/mobsted3/docker_mobsted/conf/nginx/cnamedomain/'
print('------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------')
